...
//This part combinely work with login.php page setcookie() function for time validation
	if(isset($_COOKIE['SNID'])){
		if(DB::query('SELECT user_id FROM `token` WHERE token=:token', array(':token'=>sha1($_COOKIE['SNID'])))){

			$userid=DB::query('SELECT user_id FROM `token` WHERE token=:token', array(':token'=>sha1($_COOKIE['SNID'])))[0]['user_id'];
//2nd cookie
